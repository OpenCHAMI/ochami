...
func runCoreIfaceDeleteWithRuntime(cmd *cobra.Command, opts *ifaceDeleteOptions, args []string, smdClient *smd.SMDClient, rt *cli.Runtime) error {
	...
	if !opts.NoConfirm {
		...
		if err != nil {
			...
			rt.Logger.Info().Msg("User aborted ethernet interface deletion")
			return nil
		} else {
			rt.Logger.Debug().Msg("User answered affirmatively to delete ethernet interfaces")
		}
	}
	...
	if opts.All {
		...
		if err := cli.AggregateItemErrors(rt.Logger, results.Errors(), "SMD ethernet interface deletion"); err != nil {
			return err
		}
	}
