	...
	for _, e := range results.Errors() {
		if e != nil {
			if errors.Is(e, client.UnsuccessfulHTTPError) {
				rt.Logger.Error().Err(e).Msg("SMD redfish endpoint request yielded unsuccessful HTTP response")
			} else {
				rt.Logger.Error().Err(e).Msg("failed to add redfish endpoint(s) to SMD")
			}
			errorsOccurred = true
		}
	}
