...
var cloudInitGroupAddCmd = &cobra.Command{
	...
	Long: `Add one or more new groups to cloud-init. Pass -d to pass raw
...
the data is read from standard input. If -d is not passed, the data
is read from stdin.`,
...
func init() {
	cloudInitGroupAddCmd.Flags().StringP("format-input", "f", defaultInputFormat, "format of input payload data (json,yaml)")
