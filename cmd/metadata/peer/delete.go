...
func runCoreMetadataPeerDeleteWithRuntime(cmd *cobra.Command, opts *metadataPeerDeleteOptions, args []string, metadataServiceClient *metadata_service.MetadataServiceClient, rt *cli.Runtime) error {
	// Handle token for this command
  ...
  ochami metadata peer delete --no-confirm wireguardpeer-d614b918`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if rt, ok := cli.FromContext(cmd.Context()); ok {
				...
				return runCoreMetadataPeerDeleteWithRuntime(cmd, opts, args, metadataServiceClient, rt)
			}
