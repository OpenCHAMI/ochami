...
func runCoreMetadataDefaultsDeleteWithRuntime(cmd *cobra.Command, opts *metadataDefaultsDeleteOptions, args []string, metadataServiceClient *metadata_service.MetadataServiceClient, rt *cli.Runtime) error {
	// Handle token for this command
  ...
  ochami metadata defaults delete --no-confirm clusterdefaults-d614b918`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if rt, ok := cli.FromContext(cmd.Context()); ok {
				...
				return runCoreMetadataDefaultsDeleteWithRuntime(cmd, opts, args, metadataServiceClient, rt)
			}
