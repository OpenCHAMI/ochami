...
func runCoreMetadataInstanceSet(cmd *cobra.Command, opts *metadataInstanceSetOptions, args []string, metadataServiceClient *metadata_service.MetadataServiceClient, rt *cli.Runtime) error {
	...
	rt.Logger.Debug().Msgf("instance info set: %+v", instanceSet)
...
See ochami-metadata(1) for more details.`,
  ...
  echo '<yaml_data>' | ochami metadata instance set instanceinfo-d614b918 -f yaml`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runCoreMetadataInstanceSet(cmd, opts, args, metadataServiceClient, rt)
		},
	}
	...
	cli.AddFormatInputFlag(metadataInstanceSetCmd)
