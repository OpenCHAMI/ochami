...
func runCoreBootNodeAddWithRuntime(cmd *cobra.Command, opts *bootNodeAddOptions, bootServiceClient *boot_service.BootServiceClient, rt *cli.Runtime) error {
	// Handle token for this command
  ...
  echo '<yaml_data>' | ochami boot node add -f yaml`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if rt, ok := cli.FromContext(cmd.Context()); ok {
				...
				return runCoreBootNodeAddWithRuntime(cmd, opts, bootServiceClient, rt)
			}
