...
// TestSMDCompepDeleteByData verifies IDs in a payload drive DELETE requests.
func TestSMDCompepDeleteByData(t *testing.T) {
	var deletes int
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodDelete {
			deletes++
...
func TestSMDCompepDeleteAllConfirm(t *testing.T) {
	...
	res := runOchamiWithInput(t, "y\n",
		"smd", "compep", "delete", "--uri", srv.URL, "--token", "t", "--all")
	if res.err != nil {
		t.Fatalf("unexpected error: %v (exit %d)", res.err, res.exitCode)
	}
...
func TestSMDCompepDeleteAbort(t *testing.T) {
	...
	res := runOchamiWithInput(t, "n\n",
		"smd", "compep", "delete", "--uri", srv.URL, "--token", "t", "x3000c1s7b56n0")
	if res.err != nil {
