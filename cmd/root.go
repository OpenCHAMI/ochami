...
func handleExecuteError(rootCmd *cobra.Command, err error) int {
	...
	if rt, ok := cli.FromContext(rootCmd.Context()); ok {
		rt.Logger.Error().Err(err).Msg("failed to execute command")
	}
	if cmd, _, ferr := rootCmd.Find(os.Args[1:]); ferr != nil {
		...
		if rt, ok := cli.FromContext(rootCmd.Context()); ok {
			rt.Logger.Debug().Err(ferr).Msg("failed to lookup invoked command")
		}
