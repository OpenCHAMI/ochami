...
func ReadConfigWithDefaults(path string) (*koanf.Koanf, error) {
	...
	ko, _, err := config.LoadKoanf([]config.Source{
...
func EffectiveKoanf(paths ...string) (*koanf.Koanf, error) {
	sources := make([]config.Source, 0, len(paths)+1)
	sources = append(sources, config.Source{Name: "default", Map: config.DefaultGlobalMap()})
	for i, path := range paths {
		sources = append(sources, config.Source{Name: fmt.Sprintf("path-%d", i), Path: path, Optional: true})
	}
	ko, _, err := config.LoadKoanf(sources)
	return ko, err
}
...
func writeConfig(path string, k *koanf.Koanf, ops configWriteOps) error {
	...
	if err := writeTemporaryConfig(tmp, fmode, c); err != nil {
...
// createTemporaryFile is the production adapter for creating a temporary file.
...
// temporary file. On failures before Close, it makes a best-effort Close so the
...
// DeleteCluster removes an entire cluster entry (identified by name) from the
// config file at path, clearing default-cluster if it pointed at the deleted
// cluster. An error is returned if the cluster does not exist or if reading or
// writing fails.
func DeleteCluster(path, cluster string) error {
	ko, err := ReadConfig(path)
	if err != nil {
		return fmt.Errorf("failed to read %s for modification: %w", path, err)
	}
	...
	// It doesn't make sense to delete a cluster that doesn't exist, so err
	// before writing anything back to the file.
	if !found {
		return fmt.Errorf("cluster '%s' doesn't exist", cluster)
	}
	...
	if cluster == ko.String("default-cluster") {
