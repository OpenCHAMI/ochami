...
// write to paths like /root/config.yaml that are used elsewhere in this file
// to deliberately trigger a permission-denied error.
...
func TestDeleteCluster(t *testing.T) {
	t.Run("empty path returns error", func(t *testing.T) {
		if err := DeleteCluster("", "c1"); err == nil {
			t.Fatal("DeleteCluster(): expected read error, got nil")
		}
	})
...
clusters:
  - name: a`))
		...
		if err := DeleteCluster(path, "b"); err == nil {
  ...
  - name: b
    cluster:
      uri: https://b.example`))
		...
		if err := DeleteCluster(path, "a"); err != nil {
  ...
  - name: b
    cluster:
      uri: https://b.example`))
		...
		if ko.String("default-cluster") != "b" {
			t.Errorf("default-cluster = %q, want unchanged b", ko.String("default-cluster"))
		}
	})
}
