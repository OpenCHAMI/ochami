...
//  4. Otherwise, the already-loaded effective koanf (ActiveKoanf()).
//
...
// produced.
func ResolveShowKoanf(cmd *cobra.Command) (*koanf.Koanf, error) {
	...
	case cmd.Flag("user") != nil && cmd.Flag("user").Changed:
		ko, err := configfile.ReadConfigWithDefaults(UserConfigFile)
		if err != nil {
