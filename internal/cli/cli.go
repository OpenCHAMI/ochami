...
// Execute's centralized "see '<cmd> --help'" hint (LogHelpHint) never fires
// for it. A planned later migration of the command tree is expected to let
// Cobra print usage on its own for these no-op metacommands (removing the
// need for this helper entirely), so that hint is not being reproduced here
// to avoid churn that would just be discarded.
