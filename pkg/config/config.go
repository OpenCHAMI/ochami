...
func (ccc *ConfigClusterConfig) MergeURIConfig(c ConfigClusterConfig) ConfigClusterConfig {
	...
	return ConfigClusterConfig{
		URI:             compare(ccc.URI, c.URI),
		BSS:             ConfigClusterBSS{URI: compare(ccc.BSS.URI, c.BSS.URI)},
		BootService:     ConfigClusterBootService{URI: compare(ccc.BootService.URI, c.BootService.URI)},
		CloudInit:       ConfigClusterCloudInit{URI: compare(ccc.CloudInit.URI, c.CloudInit.URI)},
		PCS:             ConfigClusterPCS{URI: compare(ccc.PCS.URI, c.PCS.URI)},
		MetadataService: ConfigClusterMetadataService{URI: compare(ccc.MetadataService.URI, c.MetadataService.URI)},
		SMD:             ConfigClusterSMD{URI: compare(ccc.SMD.URI, c.SMD.URI)},
		RCS:             ConfigClusterRCS{URI: compare(ccc.RCS.URI, c.RCS.URI)},
	}
}
...
// LoadDefaultsKoanf is like LoadDefaults but also returns the koanf instance
// that produced the Config, so a caller that needs both never has to load the
// same source twice.
func LoadDefaultsKoanf(opts ...LoadOption) (*koanf.Koanf, Config, error) {
	return LoadKoanf([]Source{{Name: "default", Map: DefaultGlobalMap()}}, opts...)
}
...
func LoadDefaults(opts ...LoadOption) (Config, error) {
	_, cfg, err := LoadDefaultsKoanf(opts...)
	return cfg, err
}

// LoadFileKoanf is like LoadFile but also returns the koanf instance that
// produced the Config, so a caller that needs both never has to load the same
// file twice.
func LoadFileKoanf(path string, opts ...LoadOption) (*koanf.Koanf, Config, error) {
	return LoadKoanf([]Source{
...
func LoadFile(path string, opts ...LoadOption) (Config, error) {
	_, cfg, err := LoadFileKoanf(path, opts...)
	return cfg, err
}
...
// LoadMergedKoanf is like LoadMerged but also returns the koanf instance that
// produced the Config, so a caller that needs both (e.g. to also render the
// effective configuration, as "config show" does) never has to re-derive the
// system/user source list and reload it independently.
func LoadMergedKoanf(opts ...LoadOption) (*koanf.Koanf, Config, error) {
...
func LoadMerged(opts ...LoadOption) (Config, error) {
	_, cfg, err := LoadMergedKoanf(opts...)
	return cfg, err
}
...
// raw effective representation, such as internal/configfile) and the
// unmarshaled Config. It is the single implementation of the
// defaults-merge-validate pipeline; callers needing only the raw koanf
// instance may discard the Config return value.
