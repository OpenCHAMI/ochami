...
// TestLoadMergedKoanf_MatchesConfig verifies that LoadMergedKoanf's koanf
// return value and Config return value are always derived from the same
// underlying load, so callers that need both (like the CLI's "config show")
// can never observe the two disagree.
func TestLoadMergedKoanf_MatchesConfig(t *testing.T) {
	tmpHome := t.TempDir()
	oldHome, had := os.LookupEnv("HOME")
	os.Setenv("HOME", tmpHome)
	if had {
	...
	ko, cfg, err := LoadMergedKoanf()
	if err != nil {
		t.Fatalf("LoadMergedKoanf failed: %v", err)
	}
	if ko == nil {
		t.Fatal("LoadMergedKoanf returned a nil koanf")
	}
	if ko.Duration("timeout") != cfg.Timeout {
		t.Errorf("koanf timeout = %s, Config.Timeout = %s; must match", ko.Duration("timeout"), cfg.Timeout)
	}
}
