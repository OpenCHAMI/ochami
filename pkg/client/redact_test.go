...
func TestRedactToken(t *testing.T) {
	origShowToken := ShowToken
	defer func() { ShowToken = origShowToken }()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ShowToken = tt.showToken
			got := RedactToken(tt.token)
			if got != tt.want {
				t.Errorf("RedactToken(%q) with ShowToken=%v = %q, want %q", tt.token, tt.showToken, got, tt.want)
			}
		})
	}
}
...
func TestRedactAuthHeaderValues(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ShowToken = tt.showToken
			got := redactAuthHeaderValues(tt.vals)
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("redactAuthHeaderValues(%v) with ShowToken=%v = %v, want %v", tt.vals, tt.showToken, got, tt.want)
			}
		})
	}
}
...
// header is truncated in debug logs by default and shown in full when ShowToken
// is enabled.
func TestMakeRequestRedactsAuthorizationInLogs(t *testing.T) {
	origShowToken := ShowToken
	origLogger := log.Logger
	defer func() {
		ShowToken = origShowToken
	...
	run := func(showToken bool) string {
		...
		oc, err := NewOchamiClient("test", ts.URL, false)
		if err != nil {
	...
	logs = run(true)
	if !strings.Contains(logs, fullToken) {
		t.Errorf("logs did not contain full token with ShowToken=true; logs: %s", logs)
	}
}
