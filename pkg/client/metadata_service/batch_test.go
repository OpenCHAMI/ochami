...
// TestRunBatch_PreservesOrder verifies the batch adapter retains the input sequence.
...
// TestRunBatch_StopsAfterCancellation verifies unattempted items are aligned
// with the parent cancellation error and their operations are not invoked.
func TestRunBatch_StopsAfterCancellation(t *testing.T) {
...
// TestMetadataServiceBatch_AllFailure verifies all-failure paths.
