...
// one is required). It does this by iteratively calling OchamiClient.GetData.
// Slices containing the client.HTTPEnvelope and error for each request is
// returned, along with a separate single error if a function error occurred.
func (cic *CloudInitClient) GetNodeGroupData(group string, ids ...string) ([]client.HTTPEnvelope, []error, error) {
	var (
		errors []error
		henvs  []client.HTTPEnvelope
	)
	...
	if len(ids) == 0 {
		return henvs, errors, fmt.Errorf("GetNodeGroupData(): at least one ID is required")
	}
	for _, id := range ids {
		var henv client.HTTPEnvelope
